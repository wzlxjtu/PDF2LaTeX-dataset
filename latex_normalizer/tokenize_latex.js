...
function removeReplicateParens(string) {
    // remove repliacted parenthese
    // These include cases like {{b}} => {b}
    const open = [];
    const prevMatch = {
        open: '',
        close: ''
    ...
    for (let i = 0; i < string.length; i++) {
        ...
        if (char === '{') {
            ...
            if (openBracket === prevMatch.open - 2 && prevMatch.close === i - 2) {
    ...
    return updatedString.filter(Boolean).join('').replace(/  /g, ' ').replace(/\\operatorname \*/g, "\\operatorname\*");
}
...
rl.on('line', function(line){
    ...
    try {
        ...
        if (process.argv[2] == "tokenize") {
            ...
            line = line.replace(/{\\rm/g, "\\mathrm{");
            line = line.replace(/{ \\rm/g, "\\mathrm{");
            line = line.replace(/\\rm{/g, "\\mathrm{");

            var tree = katex.__parse(line, {});
            buildExpression(tree, new options({}));     
            norm_str = norm_str.replace(/SSSSSS/g, '$');
            norm_str = norm_str.replace(/ S S S S S S/g, '$');

            norm_str = removeReplicateParens(norm_str)
